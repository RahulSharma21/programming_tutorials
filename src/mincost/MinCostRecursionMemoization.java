...
public class MinCostRecursionMemoization {
	...
	public static void main(String[] args) {
		...
		for (int target_energy = min_target_energy; target_energy <= max_target_energy; target_energy++) {
			...
			int[] cache = new int[target_energy + 1];
			
			Arrays.fill(cache,-2);

			int min_cost = solve(target_energy, cost, num, energy, cache);
	...
	private static int solve(int target_energy, int[] cost, int[] num,
		...
		for (int i = 0; i < energy.length; i++) {
			...
			if ((target_energy >= energy[i]) && (num[i] > 0)) {
				...
				int cst = cache[target_energy - energy[i]];
				
				if(cst == -2) {
						cst = solve(target_energy - energy[i], cost, num2, energy, cache);
						cache[target_energy - energy[i]] = cst;
				}
